...
class BackwardParticleTrajectories:
    ...
    def __init__(
        self, config_path, dt, grid_spacing, x0=None, y0=None, z0=None, f=0.0
    ):
    ...
    def _load_data(self):
        ...
        self.z_coords = self.height_agl.isel(
            Time=0, south_north=0, west_east=0
        ).values
    ...
    def _compute_velocity_gradients(self):
        ...
        self.du_dx = xr.DataArray(
            du_dx, dims=self.u.dims, coords=self.u.coords
        )
        self.du_dy = xr.DataArray(
            du_dy, dims=self.u.dims, coords=self.u.coords
        )
        self.du_dz = xr.DataArray(
            du_dz, dims=self.u.dims, coords=self.u.coords
        )

        self.dv_dx = xr.DataArray(
            dv_dx, dims=self.v.dims, coords=self.v.coords
        )
        self.dv_dy = xr.DataArray(
            dv_dy, dims=self.v.dims, coords=self.v.coords
        )
        self.dv_dz = xr.DataArray(
            dv_dz, dims=self.v.dims, coords=self.v.coords
        )

        self.dw_dx = xr.DataArray(
            dw_dx, dims=self.w.dims, coords=self.w.coords
        )
        self.dw_dy = xr.DataArray(
            dw_dy, dims=self.w.dims, coords=self.w.coords
        )
        self.dw_dz = xr.DataArray(
            dw_dz, dims=self.w.dims, coords=self.w.coords
        )
    ...
    def compute_trajectories(self):
        ...
        for t_idx in range(self.nt - 1, 0, -1):
            ...
            # Corrector step
            x_positions[valid, t_idx - 1] = (
                x_p[valid] + 0.5 * (u_p[valid] + u_p_next) * self.dt
            )
            y_positions[valid, t_idx - 1] = (
                y_p[valid] + 0.5 * (v_p[valid] + v_p_next) * self.dt
            )
            z_positions[valid, t_idx - 1] = np.maximum(
                z_p[valid] + 0.5 * (w_p[valid] + w_p_next) * self.dt, z_min
            )
        ...
        stretching_cumtrapz = cumulative_trapezoid(
            stretching_traj, dx=self.dt, axis=1, initial=0
        )
        tilting_cumtrapz = cumulative_trapezoid(
            tilting_traj, dx=self.dt, axis=1, initial=0
        )
