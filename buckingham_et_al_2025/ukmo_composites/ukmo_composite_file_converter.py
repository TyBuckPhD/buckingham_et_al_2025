...
class UKMOCompositeProcessor:
    ...
    def __init__(self, filepath):
        ...
        self.transformer_osgb_to_utm = Transformer.from_crs(
            "EPSG:27700", "EPSG:32630", always_xy=True
        )
        self.transformer_utm_to_latlon = Transformer.from_crs(
    ...
    def ingest_file(self):
        ...
        with open(self.filepath, "rb") as file_id:
            ...
            self.utm_corners = (
                self.transformer_osgb_to_utm.transform(
                    xllcorner_osgb, yllcorner_osgb
                ),
                self.transformer_osgb_to_utm.transform(
                    xtrcorner_osgb, ytrcorner_osgb
                ),
            )
    ...
    def _create_latlon_grid(self):
        ...
        if not self.utm_corners or not self.grid_size:
            raise ValueError(
                "Grid boundaries or size are missing. Please run ingest_file() first."
            )
        ...
        self.lon_grid, self.lat_grid = (
            self.transformer_utm_to_latlon.transform(utm_x_grid, utm_y_grid)
        )
    ...
    def plot_radar_composite(self, extent=None, output_file="composite.png"):
        ...
        if (
            self.radar_array is None
            or self.lat_grid is None
            or self.lon_grid is None
        ):
            self.ingest_file()
        ...
        fig, ax = plt.subplots(
            figsize=(10, 10), subplot_kw={"projection": ccrs.Mercator()}
        )
