...
class ParticleTrajectoryAnalysis:
    ...
    def plot_selected_trajectories_side_by_side(self, contour_value=9, buffer=0.015):
        ...
        gs = GridSpec(2, 1, height_ratios=[0.65, 1], hspace=0.01)
        ...
        for i, (lon, lat, z, stretching, tilting) in enumerate(zip(traj_lon, traj_lat, traj_z, traj_stretching, traj_tilting)):
            ...
            ax2d.text(lon[-1] + 0.0015, lat[-1] + 0.0025, label,
                      fontsize=18, fontweight='bold', color='black',
                      transform=ccrs.PlateCarree())
        
        tilting_range = 4e-5
        stretching_range = 4e-5  
        ...
        ticks = np.linspace(-4, 4, 5).astype(int)
    ...
    def plot_vorticity_components(self):
        ...
        for i, (idx, ax) in enumerate(zip(self.selected_indices, axes)):
            ...
            # Plotting
            x_lim = int(self.vorticity_threshold + 4)
            ax.set_ylim(-1, x_lim)
