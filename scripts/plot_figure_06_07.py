...
import numpy as np
from buckingham_et_al_2025.core.helmholtz_decomposition import (
...
class CalculateFrontalStrains:
    ...
    def __init__(self, config_path, output_fig_path, event_type):
        ...
        self.hhd = HelmholtzDecomposition(
            config_path=self.config_path, event_type=self.event_type
        )
    ...
    def run_decomposition(self):
        ...
        self.strain_threshold_list, self.frontal_strain_list, self.files = (
            self.hhd.run()
        )
    ...
    def plot_data(self):
        ...
        for i in range(len(self.strain_threshold_list)):
            ...
            if (
                len(frontal_strain) != self.num_time_steps
                or len(strain_threshold) != self.num_time_steps
            ):
            ...
            plt.plot(
                self.x_indices, strain_threshold, color=color, linestyle="--"
            )
    ...
    def save_figure(self):
        ...
        plt.savefig(
            self.output_fig_path, dpi=200, bbox_inches="tight", pad_inches=0.05
        )
        plt.show()
