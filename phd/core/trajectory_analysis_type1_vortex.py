...
class ParticleTrajectoryAnalysis:
    def __init__(self, config_path, x0_range, y0_range, z0_range, vorticity_threshold=9, buffer=15, levels=None):
        ...
        if levels is not None:
            self.vorticity_cmap = ColorbarVorticity(vorticity_type='Type 1', levels=levels, alpha=0.75, orientation='horizontal')
        else:
            self.vorticity_cmap = ColorbarVorticity(vorticity_type='Type 1', alpha=0.75, orientation='horizontal')
    ...
    def plot_selected_trajectories(self, contour_value=9, buffer=0.015, filename='default_trajectories.png'):
        ...
        gs = GridSpec(2, 1, height_ratios=[0.35, 1], hspace=-0.05)
        ...
        ax3d.text(lon_max - 0.001, lat_max, 250,
                  "0.5 km",
        ...
        for i, (lon, lat, z, stretching, tilting) in enumerate(zip(traj_lon, traj_lat, traj_z, traj_stretching, traj_tilting)):
            ...
            ax3d.text(lon[-1], lat[-1], z[-1] - 100,
                      label, fontsize=14, fontweight='bold', color='black')
            
            # On the 2D plot as well
            ax2d.text(lon[-1] + 0.004, lat[-1], label,
                      fontsize=14, fontweight='bold', color='black',
                      transform=ccrs.PlateCarree())
        
        tilting_range = 30e-5
        stretching_range = 30e-5  
        ...
        ticks = np.linspace(-30, 30, 5).astype(int)
