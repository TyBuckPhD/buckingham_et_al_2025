...
class ParticleTrajectoryAnalysis:
    ...
    def plot_selected_trajectories(self, contour_value=9, buffer=0.015, filename='default_trajectories.png'):
        ...
        gs = GridSpec(2, 1, height_ratios=[0.35, 1])
        ...
        # ---------- Right plot (3D) ----------
        ax3d.view_init(elev=17, azim=230)
        ...
        ax3d.text(lon_min - 0.002, (lat_min + lat_max)/2, z_level,
                  f"{y_distance:.1f} km",
                  verticalalignment='center', fontsize=16, fontweight='bold')
    
        ax3d.text(lon_min - 0.001, lat_max, 250,
                  "0.5 km",
